# 경주과외 (gyeongju-tutoring) — 작업 규칙

경상북도 경주시 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
## 도메인 / 배포
- 도메인: **nadoacademy.co.kr** (가비아, 2026-09-21 연결). `CNAME` 파일 있음 · 주소 `https://nadoacademy.co.kr/`
...
- 검색엔진 소유확인: 네이버 `74f30a3e3735d8c548a87ba340bab5d7ef59ddfc` · 구글 `vFHDomIDgUEuZLdlqwimXbzF3Jx60hV6BwPL1s1oC_E` — 홈 `<head>` 에 생성기가 넣는다(`regions.js` 의 `verify`). **`index.html` 에 손으로 넣지 말 것** — 재생성하면 사라진다. 코드를 바꾸려면 `verify` 를 고치고 재생성.
...
| `schools.html` | 경주 초·중·고 83곳 목록 + 검색 |
| `school/*.html` | **학교별 정적 페이지 83개** — SEO 의 핵심. 학교당 국어·영어·수학·사회·과학·코딩 키워드 |
| `teachers.html` | 선생님 (화상 673명 — 경주 방문 선생님이 0명이라 방문 관련 내용은 감춰져 있다) |
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo gyeongju-tutoring -Status 완료 -What "글 N편"`
